...
<!-- Waiting on a rerun for inflation (broken) and cbdc(didn't converge) -->

## Assessment

State Space models are much better in that they seem to catch some of the dynamics.  The sampling is notoriously challenging becuase we need to deal with latent states and relationships between states and parameters produce a complex posterior topography making good proposal distributions difficult.  **The results here are considered as demonstration only since the samples do not pass the quality control standards**.  I don't see this as being fixable other than running this for a really long time.


One alternative is to use a sampler that handles the complex posterior shape directly.  Typically this involves recoding the model into **stan** and using Hamiltonian Monte Carlo or a (related) No U-Turn sampler.
...
<!--   ggtitle("Log unnormalized posterior")+geom_vline(xintercept = niter/2) -->
...
<!-- print("sigma2_delta") -->
<!-- PT_results$theta[[which(temperatures ==1)]][1:(niter/2),"sigma2_delta"]|> summary() -->
<!-- print("sigma2_e") -->
<!-- PT_results$theta[[which(temperatures ==1)]][1:(niter/2),"sigma2_e"]|> summary() -->
...
LP|>
  ggplot()+
  geom_line(aes(x = index, y=value, color = sigma2))+
  facet_wrap(~sigma2, scales = "free")
```
...
stacked = stacked|> mutate(
  par = str_replace_all(par,
                        ...
                        replacement = paste0("A_",countries2use[country])))|> mutate(
  par = str_replace_all(par,
                        pattern = paste0("B",country),
                        replacement = paste0("B_",countries2use[country])))|> mutate(
  par = str_replace_all(par,
                        pattern = paste0("C",country),
                        replacement = paste0("C_",countries2use[country])))|> mutate(
  par = str_replace_all(par,
                        pattern = paste0("c",country),
                        replacement = paste0("c_",countries2use[country])))
}
