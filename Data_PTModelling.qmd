...
$$P(X_{t},\ldots,X_{0}\mid \Theta, \sigma^2) =  \int_\chi P(X_{0})\prod_{j=1}^J\prod_{s=1}^t \left[P(X_{jt}\mid X_{t-1}, \Theta)^{I\!I(X_{jt})}P(X_{jt}^*\mid X_{t-1}, \Theta)^{1-I\!I(X_{jt})}\right] dX_{jt}^*$$
